...
@endsection
@push('style')
    @php $site_color = get_static_option('general_site_color'); @endphp
    <style >
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.general.preloader.settings')}}" method="Post" enctype="multipart/form-data">
                            ...
                            <div class="mb-3 custom_preloader_field_wrapper" style="display: @if(!empty(get_static_option('preloader_custom'))) block @else none @endif">
                                <div class="mb-3 edit">  
                                    <label for="preloader_custom_image" class="form-label d-block"><strong >{{__('Custom Preloader Image')}}</strong></label>
                                    @php
                                        $logo = get_static_option('preloader_custom_image');
                                    @endphp    
                                    @if($logo)
                                        <img id="bgImagePreview" src="{{ asset($logo) }}" alt="Default Background" class="image-preview-container max-width-img">
                                    @else
                                        <img id="bgImagePreview" src="{{ asset('uploads/placeholder.png') }}" alt="Default Image" class="image-preview-container max-width-img">
                                    @endif                                                    
                                </div>
                                <div class="mb-3">
                                    <label for="edit_image_1" class="form-label">{{ __('Change Photo') }}</label>
                                    <input type="file" class="form-control edit_image" id="edit_image" name="preloader_custom_image">
