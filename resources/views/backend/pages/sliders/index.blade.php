    ...
    <div class="col-lg-12 col-md-12">
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('Sliders Items')}}</h4>
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <a href="{{ route('admin.sliders.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm"> <i class="ri-add-line"></i>{{ __('Add Item') }}</a>
                        </div>
                        <ul class="nav nav-tabs" id="myTab" role="tablist">
                            @foreach($all_sliders as $key => $team)
                                                    ...
                                                    <tr >
                                                        ...
                                                        <td >
                                                            <div class="photo-container">
                                                                <a href="{{ asset($data->SliderImageUrl) }}" class="magnific">
                                                                    <img src="{{ asset($data->SliderImageUrl) }}" alt="{{ $data->title }}">
                                                                </a>
                                                            </div>
                                                        </td>
                                                        <td >{{$data->title}}</td>
                                                        ...
                                                        <td >{{$data->btn_text}}</td>
                                                        <td >
                                                            @include('backend.partials.delete-with-swal', ['url' => route('admin.sliders.destroy', $data->id)])
                                                            <a href="{{ route('admin.sliders.edit', $data->id) }}" class="btn btn-primary btn-sm me-1">
                                                                <i class="ri-pencil-line"></i>
                                                            </a>
                                                            <form action="{{ route('admin.sliders.clone', $data->id) }}" method="post" style="display: inline-block">
                                                                @csrf
        ...
        $(document).ready(function() {
            $(document).on('click', '#bulk_delete_btn', function (e) {
                ...
                if (allIds.length > 0 && bulkOption == 'delete') {
                    ...
                    axios.delete(route('admin.sliders.bulk.action'), {
                        data: {
                            ids: allIds,
                            _token: "{{csrf_token()}}",
                        }
