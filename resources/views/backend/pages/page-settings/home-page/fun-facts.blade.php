...
<div class="col-lg-12 col-ml-12">
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">{{__('Fun Facts Area Settings')}}</h4>
                    <form action="{{route('admin.home-page.fun-facts')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <nav >
                        ...
                        <div class="tab-content mt-4" id="nav-tabContent">
                            @foreach($all_languages as $key => $lang)
                                <div @class(['tab-pane fade', 'show active' => $loop->first]) id="nav-home-{{$lang->slug}}" role="tabpanel" aria-labelledby="nav-home-tab">
                                    <div class="mb-3">
                                        <label class="form-label" for="fun_fucts_section_{{$lang}}_title">{{__('Title')}}</label>
                                        <input type="text" name="fun_fucts_section_{{$lang->slug}}_title" class="form-control" value="{{get_static_option('fun_fucts_section_'.$lang->slug.'_title')}}">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label" for="fun_fucts_section_{{$lang}}_sub_title">{{__('Sub Title')}}</label>
                                        <input type="text" name="fun_fucts_section_{{$lang->slug}}_sub_title" class="form-control" value="{{get_static_option('fun_fucts_section_'.$lang->slug.'_sub_title')}}">
                                    </div> 
                                    <div class="mb-3">
                                        <label class="form-label" for="fun_fucts_section_{{$lang}}_description">{{__('Description')}}</label>
                                        <textarea name="fun_fucts_section_{{$lang->slug}}_description" class="form-control" id="fun_fucts_section_{{$lang}}_description" cols="30" rows="7">{{get_static_option('fun_fucts_section_'.$lang->slug.'_description')}}</textarea>
                                    </div>    
                                </div>
                            @endforeach
                            <div class="mb-3">
                                <label class="form-label" for="title">{{__('Fun Facts Item')}}</label>
                                <select name="fun_facts_{{get_static_option('home_page_variant')}}_id[]" class="form-select select2_tags" multiple="multiple" id="tagsInput">
                                    @php
                                        $funFactsItems = 'fun_facts_' . get_static_option('home_page_variant') . '_id';
                                        $funFactsItems = json_decode(get_static_option($funFactsItems), true) ?? [];
                                    @endphp
                                    @foreach ($all_items as $item)
                                        <option value="{{ $item->id }}" @selected(in_array($item->id, $funFactsItems))>{{ $item->title }}</option>                                        
                                    @endforeach
                                </select>
                            </div>
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="mb-3 edit_1">  
                                        <label for="fun_facts_bg_image" class="form-label d-block">{{__('Backround Image')}}</label>
                                        @php
                                            $leftImage = get_static_option('fun_facts_bg_image');
                                        @endphp    
                                        @if($leftImage)
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="file" class="form-control edit_image" id="edit_image_1" name="fun_facts_bg_image">
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <div class="mb-3 edit_2">  
                                        <label for="fun_facts_float_image" class="form-label d-block">{{__('Float Image')}}</label>
                                        @php
                                            $leftImage = get_static_option('fun_facts_float_image');
                                        @endphp    
                                        @if($leftImage)
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="file" class="form-control edit_image" id="edit_image_2" name="fun_facts_float_image">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="col-lg-6">
            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">{{__('Add Fun Fact Item')}}</h4>
                    <div class="right-cotnent margin-bottom-40"><a class="btn btn-primary" data-bs-target="#add_fun_fact" data-bs-toggle="modal" href="#">{{__('Add New Fun Fact Item')}}</a></div>

                    <ul class="nav nav-tabs mt-4 mb-4" id="myTab" role="tablist">
                        @foreach($all_fun_facts as $key => $fun_fact)
                            <li class="nav-item">
                    ...
                    <div class="tab-content" id="myTabContent">
                        @foreach($all_fun_facts as $key => $fun_facts)
                                            ...
                                            @foreach($fun_facts as $data)
                                                <tr >
                                                    ...
                                                    <td >
                                                        ...
                                                        <a href="#" data-bs-toggle="modal" data-bs-target="#fun_fact_item_edit_modal" class="btn btn-xs btn-primary btn-sm me-1 fun_fact_item_edit_btn" data-id="{{$data->id}}" data-lang="{{$data->lang}}" data-title="{{$data->title}}" data-icon="{{$data->icon}}" data-counter="{{$data->counter}}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__('Add Fun Fact Item')}}</h5>
            ...
            <form action="{{ route('admin.fun-facts.store') }}" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label" for="">{{__('Counter')}}</label>
                        <input type="text" name="counter" class="form-control">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{__('Close')}}</button>
                    <button type="submit" class="btn btn-primary">{{__('Add Fun Fact Item')}}</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="fun_fact_item_edit_modal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.fun-facts.update') }}" method="post">
                <div class="modal-body">
                    @csrf
                    ...
                    <div class="mb-3">
                        <label class="form-label" for="">{{__('Counter')}}</label>
                        <input type="text" name="counter" id="fun_fact_item_counter" class="form-control">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{__('Close')}}</button>
        ...
        (function($){
            ...
            $(document).ready(function () {
                ...
                $(document).on('click','.fun_fact_item_edit_btn',function(){
                    ...
                    form.find('#fun_fact_item_counter').val(counter);
