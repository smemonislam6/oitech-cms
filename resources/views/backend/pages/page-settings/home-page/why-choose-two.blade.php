    ...
    <div class="col-lg-12 col-md-12">
        <div class="row">
            ...
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('Why Choose Section Settings')}}</h4>

                        <form action="{{route('admin.home-page.why-choose.two')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <nav >
                            ...
                            <div class="tab-content mt-4" id="nav-tabContent">
                                @foreach($all_languages as $key => $lang)
                                    <div @class(['tab-pane fade', 'show active' => $loop->first]) id="nav-home-{{$lang->slug}}" role="tabpanel" aria-labelledby="nav-home-tab">
                                        <div class="mb-3">
                                            <label for="why_choose_two_{{$lang}}_title">{{__('Title')}}</label>
                                            <input type="text" name="why_choose_two_{{$lang->slug}}_title" value="{{get_static_option('why_choose_two_'.$lang->slug.'_title')}}" class="form-control" id="why_choose_two_{{$lang->slug}}_title">
                                        </div>
                                        <div class="mb-3">
                                            <label for="why_choose_two_{{$lang}}_sub_title">{{__('Sub Title')}}</label>
                                            <input type="text" name="why_choose_two_{{$lang->slug}}_sub_title" value="{{get_static_option('why_choose_two_'.$lang->slug.'_sub_title')}}" class="form-control" id="why_choose_two_{{$lang->slug}}_sub_title">
                                        </div>
                                        <div class="mb-3">
                                            <label for="why_choose_two_{{$lang->slug}}_description">{{__('Description')}}</label>
                                            <textarea name="why_choose_two_{{$lang->slug}}_description" id="why_choose_two_{{$lang->slug}}_desciption" class="form-control editor" cols="30" rows="10">{{get_static_option('why_choose_two_'.$lang->slug.'_description')}}</textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label d-block" for="icon">{{__('Experience Icon')}}</label>
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-primary iconpicker-component">
                                                    <i class="{{get_static_option('why_choose_two_'.$lang->slug.'_icon')}}"></i>
                                                </button>
                                                <button type="button" class="icp icp-dd btn btn-primary dropdown-toggle" data-selected="{{get_static_option('why_choose_two_'.$lang->slug.'_icon') ?? 'fas fa-exclamation-triangle'}}" data-bs-toggle="dropdown">
                                                    <span class="caret"></span>
                                                    <span class="sr-only">{{ __('Toggle Dropdown') }}</span>
                                                </button>
                                                <div class="dropdown-menu"></div>
                                            </div>
                                            <input type="hidden" class="form-control" id="icon" value="{{get_static_option('why_choose_two_'.$lang->slug.'_icon') ?? 'fas fa-exclamation-triangle'}}" name="why_choose_two_{{$lang->slug}}_icon">
                                        </div>
                                        <div class="mb-3">
                                            <label for="why_choose_two_{{$lang->slug}}_experience_title">{{__('Experience Title')}}</label>
                                            <textarea name="why_choose_two_{{$lang->slug}}_experience_title" class="form-control" id="why_choose_two_{{$lang->slug}}_experience_title" cols="10" rows="3">{{ get_static_option('why_choose_two_'.$lang->slug.'_experience_title')}}</textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label for="why_choose_two_{{$lang}}_url">{{__('Video URL')}}</label>
                                            <input type="text" name="why_choose_two_{{$lang->slug}}_url" value="{{get_static_option('why_choose_two_'.$lang->slug.'_url')}}" class="form-control" id="why_choose_two_{{$lang->slug}}_url">
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="why_choose_two_{{get_static_option('home_page_variant')}}_id[]" class="form-select select2_tags" multiple="multiple" id="tagsInput">
                                    @php
                                        $whyChooseItems = 'why_choose_two_' . get_static_option('home_page_variant') . '_id';
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="mb-3 edit_1">
                                        <label for="why_choose_two_bg_image" class="form-label d-block">{{__('Backround Image')}}</label>
                                        @php
                                            $leftImage = get_static_option('why_choose_two_bg_image');
                                        @endphp
                                        @if($leftImage)
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="file" class="form-control edit_image" id="edit_image_1" name="why_choose_two_bg_image">
                                    </div>
                                </div>
                                <div class="col-lg-12">
                                    <div class="mb-3 edit_2">
                                        <label for="why_choose_two_bg_shape" class="form-label d-block">{{__('Backround Image Shape')}}</label>
                                        @php
                                            $leftImage = get_static_option('why_choose_two_bg_shape');
                                        @endphp
                                        @if($leftImage)
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="file" class="form-control edit_image" id="edit_image_2" name="why_choose_two_bg_shape">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="mb-3 edit_3">
                                        <label for="why_choose_two_float_image" class="form-label d-block">{{__('Float Image')}}</label>
                                        @php
                                            $rightImage = get_static_option('why_choose_two_float_image');
                                        @endphp
                                        @if($rightImage)
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="file" class="form-control" id="edit_image_3" name="why_choose_two_float_image">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="mb-3 edit_4">
                                        <label for="why_choose_two_image_two" class="form-label d-block">{{__('Arrow Image')}}</label>
                                        @php
                                            $rightImage = get_static_option('why_choose_two_arrow_image');
                                        @endphp
                                        @if($rightImage)
                                            <img id="arrowImagePreview" src="{{ asset($rightImage) }}" alt="Default Background" class="image-preview-container max-height-img" >
                                        @else
                                            <img id="arrowImagePreview" src="{{ asset('uploads/placeholder.png') }}" alt="Default Image" class="image-preview-container max-height-img" >
                                        @endif
                                    </div>
                                    <div class="mb-3">
                                        <label for="edit_image_4" class="form-label">{{ __('Change Photo') }}</label>
                                        <input type="file" class="form-control" id="edit_image_4" name="why_choose_two_arrow_image">
                                    </div>
                                </div>
                            </div>
