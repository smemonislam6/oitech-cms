    ...
    <div class="col-lg-12 col-md-12">
        <div class="row">
            ...
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('About Area One Settings')}}</h4>

                        <form action="{{route('admin.home-page.about.one')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <nav >
                            ...
                            <div class="tab-content mt-4" id="nav-tabContent">
                                @foreach($all_languages as $key => $lang)
                                    <div @class(['tab-pane fade', 'show active' => $loop->first]) id="nav-home-{{$lang->slug}}" role="tabpanel" aria-labelledby="nav-home-tab">
                                        <div class="mb-3">
                                            <label for="about_section_one_{{$lang}}_title">{{__('Title')}}</label>
                                            <input type="text" name="about_section_one_{{$lang->slug}}_title" value="{{get_static_option('about_section_one_'.$lang->slug.'_title')}}" class="form-control" id="about_section_one_{{$lang->slug}}_title">
                                        </div> 
                                        <div class="mb-3">
                                            <label for="about_section_one_{{$lang}}_sub_title">{{__('Sub Title')}}</label>
                                            <input type="text" name="about_section_one_{{$lang->slug}}_sub_title" value="{{get_static_option('about_section_one_'.$lang->slug.'_sub_title')}}" class="form-control" id="about_section_one_{{$lang->slug}}_sub_title">
                                        </div>
                                        <div class="mb-3">
                                            <label for="about_section_one_{{$lang->slug}}_description">{{__('Description')}}</label>
                                            <textarea name="about_section_one_{{$lang->slug}}_description" class="form-control editor" cols="30" rows="10">{{get_static_option('about_section_one_'.$lang->slug.'_description')}}</textarea>
                                        </div>   
                                        <div class="mb-3">
                                            <label for="about_section_one_{{$lang->slug}}_btn_title">{{__('Button Tittle')}}</label>
                                            <input type="text" name="about_section_one_{{$lang->slug}}_btn_title" class="form-control" value="{{get_static_option('about_section_one_'.$lang->slug.'_btn_title')}}">
                                        </div>
                                        <div class="mb-3">
                                            <label for="about_section_one_{{$lang->slug}}_btn_url">{{__('Button URL')}}</label>
                                            <input type="text" name="about_section_one_{{$lang->slug}}_btn_url" class="form-control" value="{{get_static_option('about_section_one_'.$lang->slug.'_btn_url')}}">
                                        </div>                                     
                                        <div class="mb-3">
                                            <label for="about_section_one_{{$lang->slug}}_experience_year">{{__('Experience Year')}}</label>
                                            <input type="text" name="about_section_one_{{$lang->slug}}_experience_year" class="form-control" value="{{get_static_option('about_section_one_'.$lang->slug.'_experience_year')}}">
                                        </div>
                                        <div class="mb-3">
                                            <label for="about_section_one_{{$lang->slug}}_experience_title">{{__('Experience Title')}}</label>
                                            <input type="text" name="about_section_one_{{$lang->slug}}_experience_title" class="form-control" value="{{get_static_option('about_section_one_'.$lang->slug.'_experience_title')}}">
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            <div class="mb-3">
                                <label class="form-label" for="title">{{__('About Item')}}</label>
                                <select name="about_one_{{get_static_option('home_page_variant')}}_id[]" class="form-select select2_tags" multiple="multiple" id="tagsInput">
                                    @php
                                        $aboutItems = 'about_one_' . get_static_option('home_page_variant') . '_id';
                                        $aboutItems = json_decode(get_static_option($aboutItems), true) ?? [];
                                    @endphp
                                    @foreach ($all_items as $item)
                                        <option value="{{ $item->id }}" @selected(in_array($item->id, $aboutItems))>{{ $item->title }}</option>                                        
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <div class="btn-group">
                                    ...
                                    <button type="button" class="icp icp-dd btn btn-primary dropdown-toggle" data-selected="{{get_static_option('about_section_one_icon') ?? 'fas fa-exclamation-triangle'}}" data-bs-toggle="dropdown">
                                        <span class="caret"></span>
                                        <span class="sr-only">{{ __('Toggle Dropdown') }}</span>
                                    </button>
                                    <div class="dropdown-menu"></div>
                                </div>
                                <input type="hidden" class="form-control" id="icon" value="{{get_static_option('about_section_one_icon') ?? 'fas fa-exclamation-triangle'}}" name="about_section_one_icon">
                            </div>
                            <div class="mb-3 edit">  
                                <label for="about_one_image_one" class="form-label d-block">{{__('Left Image')}}</label>
                                @php
                                    $leftImage = get_static_option('about_one_image_one');
                                @endphp    
                                @if($leftImage)
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control" id="edit_image" name="about_one_image_one">
                            </div>
                            <div class="mb-3 add">         
                                <label for="about_one_image_two" class="form-label d-block">{{__('Right Image')}}</label>
                                @php
                                    $rightImage = get_static_option('about_one_image_two');
                                @endphp                                             
                                    @if($rightImage)
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control" id="add_image" name="about_one_image_two">
                            </div>
                            <div class="mb-3 edit_3">         
                                <label for="about_one_float_image" class="form-label d-block">{{__('Float Image')}}</label>
                                @php
                                    $floatImage = get_static_option('about_one_float_image');
                                @endphp                                             
                                @if($floatImage)
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control" id="edit_image_3" name="about_one_float_image">
                            </div>
